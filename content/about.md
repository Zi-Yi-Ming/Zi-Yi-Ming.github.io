---
title: "关于我"
description: "关于这个博客和作者"
ShowReadingTime: false
ShowWordCount: false
hideAuthor: true
hideMeta: true
---

# 关于我

软件工程专业在读，关注 AI 智能体、量化研究与开源。平时用 Hugo 写博客，用 Obsidian 记笔记，折腾各种 AI 工具（Claude Code、AtomCode、CNB CodeBuddy NPC），也自己做点开源小项目。

## 关于这个博客

本站使用 **Hugo + PaperMod** 搭建，托管在 GitHub Pages 上，免费、快速、无需维护服务器。记录技术笔记、工具清单与项目实践。

## 联系方式

- **CSDN**：[blog.csdn.net/2402_87488142](https://blog.csdn.net/2402_87488142)（技术笔记与文章）
- **GitHub**：[github.com/Zi-Yi-Ming](https://github.com/Zi-Yi-Ming)（开源项目，如 cnb-npc-skill）
- **QQ**：3125727661
- **邮箱**：[ziyim2026@gmail.com](mailto:ziyim2026@gmail.com)
